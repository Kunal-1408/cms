...
export default function Input_02({
  ...
  const filteredSuggestions = suggestions
    .filter(
      (suggestion) =>
        suggestion.label.toLowerCase().includes(input.toLowerCase()) && !tags.find((tag) => tag.id === suggestion.id),
    )
    .slice(0, 5)

  const canAddNewTag = !suggestions.find((s) => s.label.toLowerCase() === input.toLowerCase()) && input.length > 0
    ...
    <div className="w-full max-w-full sm:max-w-2xl space-y-2" ref={containerRef}>
      ...
      >
        {tags.map((tag) => (
          <span
            key={tag.id}
            className={cn(tagStyles.base, "text-base sm:text-sm py-1 sm:py-0.5", tag.color || tagStyles.colors.blue)}
          >
            {tag.label}
            <button
              type="button"
        ...
        {isOpen && (input || filteredSuggestions.length > 0) && (
          ...
          >
            <div className="px-2 py-1.5 border-b border-zinc-200 dark:border-zinc-800">
              <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Choose a tag or create one</span>
            </div>
